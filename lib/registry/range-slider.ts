...
usage: `import RangeSlider from "@/app/components/ui/range-slider"
import { useState } from "react"

export function Demo() {
  const [range, setRange] = useState<[number, number]>([20, 80])
  return (
    <RangeSlider min={0} max={100} step={5} value={range} onChange={setRange} />
  )
}`,
props: [
  {
  ...
  },
  {
    ...
    type: "[number, number]",
    default: "—",
    description: "Current range as [low, high] tuple.",
  },
  {
    name: "onChange",
...
playground: {
  controls: [
    { name: "min", label: "Min", type: "number", default: 0, min: 0, max: 50 },
    { name: "max", label: "Max", type: "number", default: 100, min: 50, max: 1000 },
...
component: () => import("@/app/components/ui/range-slider"),
demo: () => import("@/app/components/demos/range-slider-demo"),
};
